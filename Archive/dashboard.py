...
class DashboardWindow(QWidget):
    def __init__(self):
        ...
        self.dropdown.addItem("Select a Chairperson")
...
class BillDetailsEntryWindow(QWidget):
    def __init__(self):
        ...
        add_button.setStyleSheet("""
            ...
            border-radius: 5px;
        """)
        add_button.setFixedSize(280, 50)
        ...
        main_horizontal_line = QFrame()
        main_horizontal_line.setFrameShape(QFrame.VLine)
        main_horizontal_line.setFrameShadow(QFrame.Sunken)
        main_horizontal_line.setLineWidth(4)

        # **Right Hand Side Layout**
        right_side_layout = QVBoxLayout()

        # Dropdown for bills
        right_title_label = QLabel("Current Bills Running")
        ...
        details_heading = QLabel("Bill Details")
        details_heading.setStyleSheet("font-size: 35px; font-weight: bold; color: #B22222;")
        details_heading.setAlignment(Qt.AlignCenter)

        self.date_tabled_label = QLabel("Date Tabled: -")
        self.date_tabled_label.setStyleSheet("font-size: 28px; font-weight: bold; color: #B22222;")

        self.name_label = QLabel("Name of Bill: -")
        self.name_label.setStyleSheet("font-size: 28px; font-weight: bold; color:#B22222;")

        self.status_label = QLabel("Status: -")
        self.status_label.setStyleSheet("font-size: 28px; font-weight: bold; color:#B22222;")

        mark_complete_button = QPushButton("Mark Bill as Complete")
        mark_complete_button.setStyleSheet("""
            font-size: 22px; 
            font-weight: bold; 
            background-color: #B22222; 
            color: white; 
            ...
            border-radius: 5px;
        """)
        mark_complete_button.clicked.connect(self.mark_bill_as_complete)

        details_layout = QVBoxLayout()
        details_layout.addWidget(right_title_label)
        details_layout.addWidget(details_heading)
        details_layout.addWidget(self.date_tabled_label)
        details_layout.addWidget(self.name_label)
        details_layout.addWidget(self.status_label)
        details_layout.addWidget(mark_complete_button)
        details_layout.addStretch()

        right_side_layout.addWidget(right_title_label)
        
        right_side_layout.addWidget(self.bill_dropdown)       
        right_side_layout.addLayout(details_layout)
        ...
        main_layout.addStretch()
        main_layout.addWidget(main_horizontal_line)
        main_layout.addStretch()
    ...
    def load_bill_details(self):
        """Load and display details of the selected bill from the dropdown."""
        selected_bill = self.bill_dropdown.currentText().strip()

        # If no bill is selected, clear the details and return
        if not selected_bill or selected_bill == "Select a Bill":
            self.date_tabled_label.setText("Date Tabled: -")
            self.name_label.setText("Name of Bill: -")
            ...
            try:
                ...
                query = """
                 ...
                 FROM bill_details
                    WHERE bill_name = %s
                """
                cursor.execute(query, (selected_bill,))
                result = cursor.fetchone()
                if result:
                    tabled_date, bill_name, status = result
                    self.date_tabled_label.setText(f"Date Tabled: {tabled_date}")
                    self.name_label.setText(f"Name of Bill: {bill_name}")
                ...
                logging.error(f"Database Error: {err}")
                QMessageBox.critical(self, "Database Error", f"Error loading bill details: {err}")
            finally:
                connection.close()

    def mark_bill_as_complete(self):
        """Mark the selected bill as complete."""
        bill_name = self.bill_dropdown.currentText()
        if not bill_name:
            QMessageBox.warning(self, "Selection Error", "No bill selected!")
            return
        ...
        if connection:
            try:
                cursor = connection.cursor()
                query = "UPDATE bill_details SET status = 'Complete' WHERE bill_name = %s"
                cursor.execute(query, (bill_name,))
                connection.commit()
                QMessageBox.information(self, "Success", f"'{bill_name}' marked as Complete.")
                self.load_running_bills()  # Refresh dropdown
                self.details_label.clear()
            except mysql.connector.Error as err:
                logging.error(f"Database Error: {err}")
                QMessageBox.critical(self, "Database Error", f"Error updating bill status: {err}")
            finally:
                connection.close()
    ...
    def add_bill_to_database(self):
        ...
        bill_name = self.bill_name_input.text().strip()
...
class MessageWindow(QWidget):
    def __init__(self):
        ...
        # **Main Layout Initialization**
        complete_layout = QVBoxLayout()
        complete_layout.addLayout(logo_and_title_layout)
        complete_layout.addLayout(back_button_layout)
        complete_layout.addStretch()
        complete_layout.addWidget(line)
        complete_layout.addWidget(footer_label)
